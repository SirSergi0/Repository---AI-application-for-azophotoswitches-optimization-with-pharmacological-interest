...
\begin{definition}
Adjacency Matrix: Matricidal representation of a molecule. If we have $\{x\}_{i=1}^n$ atoms that form our molecule, labeled from 1 to n, the adjacent matrix $A$ will contain as the element $a_{i,j}$ 1 if the atom $i$ and $j$ are connected and 0 otherwise. (We can also naturally add the possibilities of $a_{i,j}=2,3$ for double and triple bonds).\par
